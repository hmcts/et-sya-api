...
public class ManageCaseController {
    ...
    */
    ...
    @PostMapping("case-type/{caseType}/jurisdiction/{jurisdictionId}/event-type/{eventType}/case")
    @Operation(summary = "Create a new default case")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Accessed successfully"),
        @ApiResponse(responseCode = "400", description = "Bad Request"),
    ...
    public ResponseEntity<CaseDetails> createCaseWithBody(
        @RequestHeader("Authorization") String authorization,
        @PathVariable @NotNull String caseType,
        @PathVariable @NotNull String jurisdictionId,
        @PathVariable @NotNull String eventType,
        @RequestBody String caseData

    ) {
        var caseDetails = caseService.createCaseWithBody(authorization, caseType, jurisdictionId, eventType, caseData);
        return ok(caseDetails);
    }
}
